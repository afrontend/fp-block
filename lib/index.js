...
const makeMeteoriteShape = () => {
  ...
  return _.sampleSize(meteorite, 7);
};
...
const addMissile = (shuttlePanel, missilePanel) =>
  p.paint(
    missilePanel,
    _.map(p.getZeroPoints(shuttlePanel), point => ({ ...point, zeroPoint: true })),
    GLOBAL.missileColor
  );
...
const key = (keyName, state) =>
  isValidKey(keyName, shuttleKeyFnList)
    ? _.find(shuttleKeyFnList, item => _.isEqual(item.key, keyName)).fn(state)
    : state;
