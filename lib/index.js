...
const createShuttlePanel = (rows, columns) =>
  flow([paintShuttle, p.adjustToCenter, p.adjustToBottom])(
    p.createPanel(rows, columns),
  );

const createMeteoritePanel = (rows, columns) =>
  flow([paintRandomMeteorite, p.adjustToRandomCenter])(
    p.createPanel(rows, columns),
  );
...
const diff = (state) => ({
  ...
  meteoritePanel: p.down(p.sub(state.meteoritePanel, state.missilePanel)),
});

const checkCollisionWithMissileAndMeteorite = flow([
  diff,
