...
const createPanel = (() => {
  ...
  let savedColumns = 0;
  return (rows, columns) => {
    savedRows = rows ?? savedRows;
    savedColumns = columns ?? savedColumns;
...
const createMeteoritePanel = flow([
  createPanel,
  paintMeteorite,
...
const spaceKey = (state) => {
  // side effect: toggles GLOBAL.pause
...
const isValidKey = (key, fnList) => fnList.some((item) => item.key === key);
...
const checkCollisionWithMissileAndMeteorite = flow([diff, checkMeteoritePanel]);
...
const key = (keyName, state) =>
  isValidKey(keyName, shuttleKeyFnList)
    ? shuttleKeyFnList.find((item) => item.key === keyName).fn(state)
    : state;
